...
contract GSWSafeDeployer {
    ...
    constructor(address _superAdmin, address _admin, address _fundsReceiver) {
    ...
    function setFundsReceiver(address newReceiver) external whenPaused onlySuperAdmin {
    ...
    /**
     * @notice Deploy a new GSWSafeV2 with an admin-signed voucher.
    ...
    function deploySafe(
        ...
        _verifyVoucher(owners, threshold, currency, amount, userNonce, deadline, adminSignature);

        // Check not already deployed
        {
            address predicted = predictSafeAddress(owners, threshold, msg.sender, userNonce);
        ...
        safe = address(
            new GSWSafeV2{salt: salt}(owners, threshold)
        );
    ...
    /**
     * @notice Deploy a new GSWSafeV2 without charging any fee. Only callable by admin.
     */
