...
contract GSWSafeV3 {
    ...
    bytes32 public constant ADD_OWNER_TYPEHASH = keccak256(
        "AddOwner(address owner,uint256 newThreshold,uint256 nonce)"
    );
    bytes32 public constant REMOVE_OWNER_TYPEHASH = keccak256(
        "RemoveOwner(address owner,uint256 newThreshold,uint256 nonce)"
    );
    bytes32 public constant SET_THRESHOLD_TYPEHASH = keccak256(
        "SetThreshold(uint256 newThreshold,uint256 nonce)"
    );
    ...
    event BackupSet(address indexed owner, address indexed backup, uint256 inactivityBlocks);
    event BackupRemoved(address indexed owner, address indexed backup);
    event BackupActivated(address indexed replacedOwner, address indexed newOwner);
    event DelegateSet(address indexed owner, address indexed delegate, uint256 nonce, uint256 expiry);
    event DelegateRevoked(address indexed owner, address indexed delegate);
    event DelegateUsed(address indexed owner, address indexed delegate, uint256 indexed nonce);
    ...
    constructor(address[] memory _owners, uint256 _threshold, uint256 _minInactivityBlocks) {
    ...
    function addOwner(address owner, uint256 newThreshold, bytes calldata signatures) external nonReentrant {
        ...
        if (backupPrincipal != address(0) && isOwner[backupPrincipal]) revert InvalidOwner();
        ...
        uint256 currentNonce = nonce;
        bytes32 structHash = keccak256(abi.encode(
            ADD_OWNER_TYPEHASH, owner, newThreshold, currentNonce
        ));
        bytes32 hash = _hashTypedData(structHash);
    ...
    function removeOwner(address owner, uint256 newThreshold, bytes calldata signatures) external nonReentrant {
        ...
        uint256 currentNonce = nonce;
        bytes32 structHash = keccak256(abi.encode(
            REMOVE_OWNER_TYPEHASH, owner, newThreshold, currentNonce
        ));
        bytes32 hash = _hashTypedData(structHash);
    ...
    function setThreshold(uint256 newThreshold, bytes calldata signatures) external nonReentrant {
        ...
        uint256 currentNonce = nonce;
        bytes32 structHash = keccak256(abi.encode(
            SET_THRESHOLD_TYPEHASH, newThreshold, currentNonce
        ));
        bytes32 hash = _hashTypedData(structHash);
    ...
    function cancelNonce(bytes calldata signatures) external nonReentrant {
        uint256 currentNonce = nonce;
        bytes32 structHash = keccak256(abi.encode(
            CANCEL_NONCE_TYPEHASH, currentNonce
        ));
        bytes32 hash = _hashTypedData(structHash);
    ...
    function migrate(
        ...
        len = erc721s.length;
        for (uint256 i = 0; i < len;) {
            IERC721Minimal(erc721s[i].token).safeTransferFrom(address(this), to, erc721s[i].tokenId);
            unchecked { ++i; }
        }
    ...
    function setDelegate(address delegate, uint256 _nonce, uint256 expiry) external {
        if (!isOwner[msg.sender]) revert NotOwner();
        ...
        if (expiry > block.timestamp + MAX_DEADLINE_DURATION) revert DeadlineTooLong();
    ...
    function getDelegateForNonce(address owner, uint256 _nonce) external view returns (address) {
    ...
    /// @dev Replaces `oldOwner` with `newOwner` in the owners set. Called when a backup
    ///      wallet's first use triggers the automatic owner swap.
    ...
    function _validateSignatures(bytes32 hash, bytes calldata signatures, uint256 currentNonce) internal {
        uint256 _threshold = threshold;
        if (signatures.length != _threshold * 65) revert InvalidSignatureLength();

        // Backup activations are deferred until after the full loop to prevent a mid-loop
        // isOwner mutation from allowing a backup to submit the same signature twice and
        // satisfy multiple threshold slots (privilege escalation via double-vote).
        address[] memory pendingBackupSigners = new address[](_threshold);
        ...
        for (uint256 i = 0; i < _threshold;) {
            ...
            } else if (backupFor[signer] != address(0)) {
                // Backup wallet activating after owner inactivity — defer state mutation
                address principal = backupFor[signer];
                if (!isOwner[principal]) revert NotOwner();
                if (block.number < lastActiveBlock[principal] + inactivityBlocks[principal]) revert BackupNotYetActive();
                effective = principal;
                pendingBackupSigners[pendingCount] = signer;
            ...
            // backup/delegate signatures by their principal's address, not their own.
            if (effective <= prev) revert SignaturesNotSorted();
            prev = effective;
            unchecked { ++i; }
        }

        // Apply all backup owner swaps now that signature validation is complete
        for (uint256 j = 0; j < pendingCount;) {
    ...
    function _safeERC20Transfer(address token, address to, uint256 amount) internal {
        ...
        if (!ok || (ret.length > 0 && !abi.decode(ret, (bool)))) revert ExecutionFailed();
    }
    ...
    function getAddOwnerHash(address owner, uint256 newThreshold) external view returns (bytes32) {
        return _hashTypedData(keccak256(abi.encode(ADD_OWNER_TYPEHASH, owner, newThreshold, nonce)));
    }

    function getRemoveOwnerHash(address owner, uint256 newThreshold) external view returns (bytes32) {
        return _hashTypedData(keccak256(abi.encode(REMOVE_OWNER_TYPEHASH, owner, newThreshold, nonce)));
    }

    function getSetThresholdHash(uint256 newThreshold) external view returns (bytes32) {
        return _hashTypedData(keccak256(abi.encode(SET_THRESHOLD_TYPEHASH, newThreshold, nonce)));
    }
